...
public class PieChartRenderer extends DataRenderer {
    ...
    protected void drawDataSet(Canvas c, IPieDataSet dataSet) {
        ...
        for (int j = 0; j < entryCount; j++) {
            ...
            float cx = circleBox.centerX();
            float cy = circleBox.centerY();

            RectF smallCircleBox = new RectF();
            float shift = dataSet.getSelectionShift();

            smallCircleBox.set(cx - secondHoleRadius + shift,
                    cy - secondHoleRadius + shift,
                    cx + secondHoleRadius - shift,
                    cy + secondHoleRadius - shift);
    ...
    private void drawArc(float angle, float rotationAngle, float phaseY, RectF circleBox, MPPointF center, float radius, boolean drawInnerArc, int visibleAngleCount, float sliceSpace, Paint paint, float sliceAngle, float innerRadius) {
        ...
        mBitmapCanvas.drawPath(mPathBuffer, paint);
    }
