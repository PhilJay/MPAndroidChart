...
public abstract class BarLineChartBase extends Chart {
	...
	/**
	 * the modulus that indicates if a value at a specified index in an array(list) is drawn or not. If index % modulus
	 * == 0 DRAW, else dont draw.
	 */
	protected int mLegendGridModulus = 1;

	/**
	 * indicates how many digits should be used for the y-legend, -1 means automatically calculate
	 */
	 ...
	 */
	protected static final double POW_10[] = { 0.00001, 0.0001, 0.001, 0.01, 0.1, 1, 10, 100, 1000, 10000, 100000,
		...
		for (int i = 0; i < positions.length; i += 2) {
			if (mYLegend[i / 2] != null) {
				...
				if (mDrawUnitInLegend) {
					mDrawCanvas.drawText(mFormatYLegend.format(mYLegend[i / 2]) + mUnit, positions[i] - 10, yPos,
							mYLegendPaint);
				} else {
					mDrawCanvas
							.drawText(mFormatYLegend.format(mYLegend[i / 2]), positions[i] - 10, yPos, mYLegendPaint);
				}
			}
		}
	}
	...
	/**
	 * determines how much space (in percent of the total range) is left between the loweset value of the chart and its
	 * bottom (bottomSpace) and the highest value of the chart and its top (topSpace) recommended: 3-25 % NOTE: the
	 * bottomSpace will only apply if "startAtZero" is set to false
	 * 
	 * @param bottomSpace
	 * @param topSpace
	 */
	...
	/**
	 * set to true to round all y-legend values if true, the y-legend values will be rounded - the y-legend entry count
	 * will only be approximated this way
	 * 
	 * @param enabled
	 */
	...
	public int getIndexByTouchPoint(float x, float y) {
		...
		// touch out of chart
		if (this instanceof LineChart && (touchPointIndex < 0 || touchPointIndex > getValueCount() - 1))
			return -1;
		if (this instanceof BarChart && (touchPointIndex < 0 || touchPointIndex > getValueCount()))
			return -1;
	...
	/**
	 * sets the number of y-legend digits to use, if set to -1, digits will be automatically calculated
	 * 
	 * @param digits
	 */
	...
	/**
	 * returns the number of y-legend digits that is set to use, -1 means auto calculate
	 * 
	 * @return
	 */
