...
public abstract class BarLineChartBase extends Chart {
	...
	/**
	 * width of the x-legend in pixels - this is calculated by the calcTextWidth() method
	 */
	protected int mXLegendWidth = 1;

	/**
	 * the modulus that indicates if a value at a specified index in an array(list) for the x-legend is drawn or not. If
	 * index % modulus == 0 DRAW, else dont draw.
	 */
	...
	/**
	 * if set to true, the x-legend entries will adjust themselves when scaling the graph
	 */
	...
	/**
	 * if set to true, the highlight indicator (lines for linechart, dark bar for barchart) will be drawn upon selecting
	 * values.
	 */
	protected boolean mHighLightIndicatorEnabled = true;

	/**
	 * boolean to indicate if user drawing on chart should automatically be finished
	 */
	...
	/**
	 * the object representing the y-legend, this object is prepared in the prepareYLegend() method
	 */
	...
	/**
	 * calculates the required number of digits for the y-legend and for the values that might be drawn in the chart (if
	 * enabled)
	 */
	...
	/**
	 * Sets up the y-legend. Computes the desired number of labels between the two given extremes. Unlike the
	 * prepareXLegend() method, this method needs to be called upon every refresh of the view.
	 * 
	 * @return
	 */
	...
	protected void drawOutline() {
		...
		mDrawCanvas.drawRect(mOffsetLeft, mOffsetTop, getWidth() - mOffsetRight, getHeight() - mOffsetBottom,
				mOutLinePaint);
	}
	...
	/**
	 * Sets the minimum scale values for both axes. Scale 0.5f means 0.5x zoom (zoomed out by factor 2), scale 0.1f
	 * means maximum zoomed out by factor 10, scale 2f means the user cannot zoom out further than 2x zoom, ...
	 * 
	 * @param scaleXmin
	 * @param scaleYmin
	 */
	...
	/**
	 * set this to true to enable drawing the top y-legend entry. Disabling this can be helpful when the y-legend and
	 * x-legend interfear with each other. default: true
	 * 
	 * @param enabled
	 */
	...
	/**
	 * Sets the effective range of y-values the chart can display. If this is set, the y-range is fixed and cannot be
	 * changed. This means, no recalculation of the bounds of the chart concerning the y-axis will be done when adding
	 * new data. To disable this, provide Float.NaN as a parameter or call resetYRange();
	 * 
	 * @param minY
	 * @param maxY
	 * @param invalidate
	 *            if set to true, the chart will redraw itself after calling this method
	 */
	...
	/**
	 * Resets the previously set y range. If new data is added, the y-range will be recalculated.
	 * 
	 * @param invalidate
	 *            if set to true, the chart will redraw itself after calling this method
	 */
	...
	/**
	 * if set to true, the x-legend entries will adjust themselves when scaling the graph default: true
	 * 
	 * @param enabled
	 */
	...
	/**
	 * If set to true, the highlight indicators (two lines for linechart, dark bar for barchart) will be drawn upon
	 * selecting values. Default: true
	 * 
	 * @param enabled
	 */
	...
	/**
	 * Returns the x and y values in the chart at the given touch point (encapsulated in a PointD). This method
	 * transforms pixel coordinates to coordinates / values in the chart. This is the opposite method to
	 * getPixelsForValues(...).
	 * 
	 * @param x
	 * @param y
	...
	/**
	 * Transforms the given chart values into pixels. This is the opposite method to getValuesByTouchPoint(...).
	 * 
	 * @param x
	 * @param y
	 * @return
	...
	/**
	 * returns the y-value at the given touch position (must not necessarily be a value contained in one of the
	 * datasets)
	 * 
	 * @param x
	 * @param y
	...
	/**
	 * Enables data filtering for the chart data, filtering will use the default Approximator. What filtering does is
	 * reduce the data displayed in the chart with a certain tolerance. This can especially be important concerning
	...
	/**
	 * Enables data filtering for the chart data, filtering will use the user customized Approximator handed over to
	 * this method.
	 * 
	 * @param a
	 */
	...
	/**
	 * if set to true, both x and y axis can be scaled with 2 fingers, if false, x and y axis can be scaled separately.
	 * default: false
	 * 
	 * @param enabled
	 */
