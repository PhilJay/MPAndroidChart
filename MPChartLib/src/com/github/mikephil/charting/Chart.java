...
public abstract class Chart extends View {
	...
	/**
	 * paint objects used for drawing (e.g. the bars of a chart) usually, each paint object has a different color
	 */
	...
	/**
	 * Prepares all the paint objects that are used for drawing the lines, bars or pie-slices. The number of paint
	 * objects used depends on the number of colors in the colortemplate. If more values than colors need to be drawn,
	 * colors are repeated.
	 * 
	 * @param ct
	 */
	...
	public boolean valuesToHighlight() {
		return mIndicesToHightlight == null || mIndicesToHightlight.length == 0 || mIndicesToHightlight[0] < 0 ? false
				: true;
	}
	...
	protected void drawMarkerView() {
		...
		if (mMarkerView == null || !mDrawMarkerView || !valuesToHighlight() || mIndicesToHightlight[0] < 0
				|| mIndicesToHightlight[0] >= mData.getYValSize())
			return;

		int index = mIndicesToHightlight[0];
		float value = mData.getYVals().get(index).getVal();

		// position of the marker depends on selected value index and value
		float[] pts = new float[] { index, value };
	...
	public float getAverage() {
		return getYValueSum() / mData.getYValSize();
	}
	...
	/**
	 * set a new paint object for the specified parameter in the chart e.g. Chart.PAINT_LINE
	 * 
	 * @param p
	 *            --> Chart.PAINT_LINE, Chart.PAINT_GRID, Chart.PAINT_VALUES, ...
	 * @param which
	 */
	...
	public Series getSeries(int index) {
		return mData.getYVals().get(index);
	}
	...
	public String getXValue(int index) {
		if (mData == null || mData.getXValSize() <= index)
			return null;
		else
	...
	public float getYValue(int index) {
		if (mData == null || mData.getYValSize() <= index)
			return 0f;
		else
			return mData.getYVals().get(index).getVal();
	}
