...
public class PieChart extends Chart {
	...
	/**
	 * variable for the text that is drawn in the center of the pie-chart. If this value is null, the default is
	 * "Total Value\n + getYValueSum()"
	 */
	...
	/**
	 * set this to true to draw the x-values next to the values in the pie slices
	 */
	...
	protected void prepareContentRect() {
		...
		// the pie-chart)
		mCircleBox = new RectF(width / 2 - diameter / 2 + mShift, height / 2 - diameter / 2 + mShift + mOffsetTop,
				width / 2 + diameter / 2 - mShift, height / 2 + diameter / 2 - mOffsetBottom - mShift);
	}
	...
	protected void drawHighlights() {
		...
		if (mHighlightEnabled && valuesToHighlight()) {
			...
			for (int i = 0; i < mIndicesToHightlight.length; i++) {
				...
				mDrawCanvas.drawArc(highlighted, angle, sliceDegrees, true, mDrawPaints[index % mDrawPaints.length]);
			}
		}
	}
	...
	private void drawHole() {
		...
		if (mDrawHole) {
			...
			mDrawCanvas.drawCircle(mContentRect.width() / 2, mContentRect.height() / 2, getDiameter() / 4, mHolePaint);
		}
	}
	...
	protected void drawValues() {
		...
		for (int i = 0; i < mData.getYValCount(); i++) {
			...
			if (mDrawXVals && mDrawYValues) {
				...
				mDrawCanvas.drawText(mData.getXVals().get(i), x, y + lineHeight, mValuePaint);

			} else if (mDrawXVals && !mDrawYValues) {
				mDrawCanvas.drawText(mData.getXVals().get(i), x, y, mValuePaint);
			} else if (!mDrawXVals && mDrawYValues) {
	...
	/**
	 * returns an integer array of all the different angles the chart slices have the angles in the returned array
	 * determine how much space (of 360°) each slice takes
	 * 
	 * @return
	 */
	...
	/**
	 * set a new starting angle for the pie chart (0-360) default is 0° --> right side (EAST)
	 * 
	 * @param angle
	 */
	...
	/**
	 * sets the text that is displayed in the center of the pie-chart. By default, the text is
	 * "Total Value + sumofallvalues"
	 * 
	 * @param text
	 */
	...
	/**
	 * returns the angle relative to the chart center for the given point on the chart in degrees. The angle is always
	 * between 0 and 360°, 0° is EAST
	 * 
	 * @param x
	 * @param y
	 * @return
